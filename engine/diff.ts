...
interface FacetEntry {
  ...
  facetType: string
  facet: Facet
  /** True for entries stored under `customFacets` rather than `facets`. The
   *  merge engine uses this to pick the correct write path. */
  isCustom: boolean
}
 ...
 *
 * Indexes BOTH standard facets (under `facets[ft]`) AND custom facets (under
 * `customFacets[type]`). Custom facets include canvasNode/canvasEdge — without
 * them in the index, three-way merge would silently drop position changes
 * during sync conflicts.
 */
export function collectAllFacets(root: RootContext): Map<string, FacetEntry> {
  ...
  for (const ft of getRegisteredFacetKeys()) {
    for (const f of root.facets[ft] as Facet[]) {
      map.set(f.uri, { containerId: root.uri, facetType: ft, facet: f, isCustom: false })
    }
    for (const ctx of Object.values(root.contexts)) {
      for (const f of ctx.facets[ft] as Facet[]) {
        map.set(f.uri, { containerId: ctx.uri, facetType: ft, facet: f, isCustom: false })
      }
    }
  }

  // Custom facets (canvasNode, canvasEdge, user-defined types). These live in
  // `customFacets[type]` rather than `facets[ft]`; the diff engine treats
  // them identically once indexed. The bridge stores all canvas-position
  // facets here, so this path is what makes positions sync-safe.
  for (const [type, arr] of Object.entries(root.customFacets ?? {})) {
    for (const f of arr) {
      map.set(f.uri, { containerId: root.uri, facetType: type, facet: f, isCustom: true })
    }
  }
  for (const ctx of Object.values(root.contexts)) {
    for (const [type, arr] of Object.entries(ctx.customFacets ?? {})) {
      for (const f of arr) {
        map.set(f.uri, { containerId: ctx.uri, facetType: type, facet: f, isCustom: true })
      }
    }
  }
  return map
...
const CONTEXT_EXCLUDE_KEYS = new Set([
  ...
  'facets', 'symbols',
  // Custom facets (canvasNode/canvasEdge, user-defined types) - now diffed
  // per-instance via collectAllFacets, so excluding here prevents the context
  // from being flagged as "modified" purely because a child canvas node moved.
  'customFacets',
