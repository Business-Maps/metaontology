...
describe('threeWayMerge', () => {
  ...
  // ── customFacets (canvasNode position fidelity) ───────────────────────────
  //
  // Canvas positions are stored as canvasNode Things in customFacets.canvasNode.
  // The diff/merge engine MUST treat these as first-class facets — otherwise
  // sync conflicts silently drop position changes.

  describe('customFacets (canvas positions)', () => {
    function addCanvasNode(root: RootContext, uri: string, x: number, y: number, representedBy = 'entity-x'): void {
      const r = root as RootContext & { customFacets?: Record<string, any[]> }
      if (!r.customFacets) r.customFacets = {}
      if (!r.customFacets.canvasNode) r.customFacets.canvasNode = []
      r.customFacets.canvasNode.push({
        uri, name: `node:${representedBy}`,
        x, y, width: 200, height: 100, zIndex: 0,
        representedBy,
      })
    }

    it('only theirs moved a canvasNode → merged keeps theirs position', () => {
      const base = makeRoot()
      addCanvasNode(base, 'cn-1', 0, 0)

      const ours = clone(base)
      const theirs = clone(base)
      ;(theirs as any).customFacets.canvasNode[0].x = 100
      ;(theirs as any).customFacets.canvasNode[0].y = 20

      const result = threeWayMerge(opts(base, ours, theirs))
      expect(result.success).toBe(true)
      const merged = (result.mergedModel as any).customFacets.canvasNode[0]
      expect(merged.x).toBe(100)
      expect(merged.y).toBe(20)
    })

    it('ours moves node A, theirs moves DIFFERENT node B → both moves preserved', () => {
      const base = makeRoot()
      addCanvasNode(base, 'cn-A', 0, 0, 'thing-A')
      addCanvasNode(base, 'cn-B', 0, 0, 'thing-B')

      const ours = clone(base)
      ;(ours as any).customFacets.canvasNode.find((n: any) => n.uri === 'cn-A').x = 100

      const theirs = clone(base)
      ;(theirs as any).customFacets.canvasNode.find((n: any) => n.uri === 'cn-B').y = 200

      const result = threeWayMerge(opts(base, ours, theirs))
      expect(result.success).toBe(true)
      const nodes = (result.mergedModel as any).customFacets.canvasNode
      const a = nodes.find((n: any) => n.uri === 'cn-A')
      const b = nodes.find((n: any) => n.uri === 'cn-B')
      expect(a.x).toBe(100)
      expect(b.y).toBe(200)
    })

    it('both move SAME node to different positions → conflict surfaced', () => {
      const base = makeRoot()
      addCanvasNode(base, 'cn-1', 0, 0)

      const ours = clone(base)
      ;(ours as any).customFacets.canvasNode[0].x = 50
    ...
    it('both move SAME node to the SAME position → no conflict', () => {
      const base = makeRoot()
      addCanvasNode(base, 'cn-1', 0, 0)

      const ours = clone(base)
      ;(ours as any).customFacets.canvasNode[0].x = 100
    ...
    it('ours adds canvasNode A, theirs adds canvasNode B → both kept', () => {
      const base = makeRoot()

      const ours = clone(base)
      addCanvasNode(ours, 'cn-A', 100, 20, 'thing-A')

      const theirs = clone(base)
      addCanvasNode(theirs, 'cn-B', 200, 40, 'thing-B')

      const result = threeWayMerge(opts(base, ours, theirs))
      expect(result.success).toBe(true)
      const nodes = (result.mergedModel as any).customFacets?.canvasNode ?? []
      expect(nodes.find((n: any) => n.uri === 'cn-A')).toBeTruthy()
      expect(nodes.find((n: any) => n.uri === 'cn-B')).toBeTruthy()
    })

    it('exact numeric fidelity: -123.456, 0.30000000000000004 round-trip', () => {
      const base = makeRoot()
      addCanvasNode(base, 'cn-1', 0, 0)

      const theirs = clone(base)
      ;(theirs as any).customFacets.canvasNode[0].x = -123.456
      ;(theirs as any).customFacets.canvasNode[0].y = 0.30000000000000004

      const result = threeWayMerge(opts(base, clone(base), theirs))
      const merged = (result.mergedModel as any).customFacets.canvasNode[0]
      expect(merged.x).toBe(-123.456)
      expect(merged.y).toBe(0.30000000000000004)
    })

    it('canvasEdge handle change preserved through merge', () => {
      const base = makeRoot()
      const r = base as RootContext & { customFacets?: Record<string, any[]> }
      r.customFacets = { canvasEdge: [{
        uri: 'ce-1', name: 'edge:link-1',
        sourceHandle: 'right', targetHandle: 'left', representedBy: 'link-1',
      }] }

      const theirs = clone(base)
      ;(theirs as any).customFacets.canvasEdge[0].sourceHandle = 'top'

      const result = threeWayMerge(opts(base, clone(base), theirs))
      expect(result.success).toBe(true)
      const merged = (result.mergedModel as any).customFacets.canvasEdge[0]
      expect(merged.sourceHandle).toBe('top')
      expect(merged.targetHandle).toBe('left')
    })
  })
})
